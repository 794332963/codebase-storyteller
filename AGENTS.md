...
2. 读取根目录的 `config.local.yaml`，确认仓库绝对路径和默认编辑器；不要把本机配置写回公开版 `SKILL.md`。
3. 当 `target_branch=auto` 时，先解析目标仓库的默认分支；然后通过 `git show`、`git ls-tree`、`git grep` 或等价只读 Git 操作读取目标分支，不得切换用户当前分支。
